...
function RouteComponent() {
  ...
  const close = useMutation({ ...closeMutation(), onSuccess: refresh })
  ...
  return <RoutePanel><LoadingOrError pending={query.isPending} error={query.isError} retry={() => query.refetch()} />{query.data && <form className="grid gap-5" onSubmit={(event) => { event.preventDefault(); update.mutate({ path: { id: edicionId }, body: { name: form.name, startDate: form.startDate || undefined, endDate: form.endDate || undefined, maxCapacity: form.maxCapacity ? Number(form.maxCapacity) : undefined } }) }}>
    <div className="flex items-center justify-between"><span className="rounded-full border px-3 py-1 text-xs">{query.data.status ? statusLabels[query.data.status] : "—"}</span><div className="flex flex-wrap gap-2">{query.data.status !== "ACTIVE" && query.data.status !== "CLOSED" && <Button type="button" size="sm" variant="outline" disabled={busy} onClick={() => activate.mutate({ path: { id: edicionId } })}>Activar</Button>}{query.data.status === "ACTIVE" && <Button type="button" size="sm" variant="outline" disabled={busy} onClick={() => suspend.mutate({ path: { id: edicionId } })}>Suspender</Button>}{query.data.status !== "CLOSED" && <Button type="button" size="sm" variant="outline" disabled={busy} onClick={() => close.mutate({ path: { id: edicionId } })}>Cerrar</Button>}</div></div>
