...
function RouteComponent() {
  ...
  return <RoutePanel><form className="mb-6 grid gap-3 rounded-lg border p-4" onSubmit={(event) => { event.preventDefault(); create.mutate({ path: { editionId: edicionId }, body: { type, value, description: description || undefined } }) }}><h3 className="font-semibold">Agregar requisito</h3><div className="grid gap-3 sm:grid-cols-2"><FormField label="Tipo"><Select value={type} onValueChange={(nextType) => setType(nextType as typeof type)}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(requirementLabels).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select></FormField><FormField label="Valor"><input className={inputClass} value={value} onChange={(e) => setValue(e.target.value)} required /></FormField></div><FormField label="Descripción"><input className={inputClass} value={description} onChange={(e) => setDescription(e.target.value)} /></FormField><div className="flex justify-end"><Button type="submit" disabled={create.isPending}>Agregar</Button></div></form>
