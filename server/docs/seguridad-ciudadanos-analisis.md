...
- Las nuevas rutas `/api/applications` también comprueban que el usuario esté activo y conserve el rol activo asignado, y siempre filtran por su identidad interna. Sus permisos son `applications:own:create` y `applications:own:view`; el init los asigna a `CIUDADANO` y conserva a `ADMIN` como superusuario técnico. Ver `solicitudes-beneficios.md` para el contrato completo.
