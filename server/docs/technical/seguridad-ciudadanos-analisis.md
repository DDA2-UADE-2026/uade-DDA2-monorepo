...
- `POST /api/admin/applications` requiere `applications:management:create` en el rol activo. Valida al actor autenticado y permite indicar un titular existente mediante `userId`, sin restricción de jurisdicción ni integración con Ciudadanos. Guarda `registeredByUserId` desde el JWT y audita al administrativo; no admite elegir el registrante ni habilita por sí mismo consultas ajenas. El nuevo permiso no se asigna a `CIUDADANO`. Ver `solicitudes-beneficios.md` para el contrato completo.
- `GET /api/admin/applications` y `GET /api/admin/applications/{id}` requieren `applications:management:view` en el rol activo y comprueban al actor igual que las rutas propias. Son de solo lectura y deliberadamente globales: no filtran por titular ni por registrante. Exponen el nombre y el correo del titular tomados de `users`, y en el detalle también la clave de idempotencia y su hash, que la vista propia oculta. El permiso no se asigna a `CIUDADANO` y debe concederse explícitamente a los roles administrativos que lo necesiten.
